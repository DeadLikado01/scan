...
thephuckinstart:
echo "\n";
userinput("Introduce La Web A Escanear:");
$ip = trim(fgets(STDIN, 1024));
if ($ip == "help")
  {
    echo "\n\n[+] HACK RED ++Ayuda++ [+] \n\n";
    echo $bold . $lblue . "Comandos\n";
    echo "========\n";
    echo $fgreen . "[1] help:$cln Ver el Menu de Ayuda\n";
    echo $bold . $fgreen . "[2] fix:$cln Instalar Todos Los Modulos Necesarios (Indica cuando esta en Curso la herramienta en el primer tiempo)\n";
    echo $bold . $fgreen . "[3] URL:$cln Ingresar El nombre url de la WEB a Escanear (Formato:www.ejemplo.com / ejemplo.com)\n";
   echo $bold . $fgreen . "[4] INFO:$cln Creado por DeadLikado Hack\n";
    goto thephuckinstart;
  }
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] HACK SCAN MENU [+]\n\n$cln";
    ...
    echo $bold . $fgreen . "[i] Funcion Terminada Exito! Porfabor Entra de Nuevo a HACK SCAN\n";
    exit;
  }
...
elseif (strpos($ip, ' ') !== false)
  {
    echo $bold . $red . "\n[!] URL FORMATO INVALIDA! Toca enter Para validar URL \n" . $cln;
    goto thephuckinstart;
  }
else
  {
    echo "\n";
    userinput("Enter 1 Para HTTP O Enter 2 Para HTTPS");
...
scanlist:
    ...
    echo $bold . $magenta . "
           [√] H E R R A M I E N T A S [√]
             ...
             [√] S C A N E R   W E B [√]
	       [√] D E A D L I K A D O [√]
          [√] BY S A M U E L  R A M I R E Z [√]
            $lgreen Escanear Sitio : " . $yellow . $ipsl . $ip . $blue . "
      \n\n";
    echo $blue. " [0]  Scaner Basico$white (Scanear IP, etc...)$blue \n [1]  Whois Lookup \n [2]  Geo-IP Lookup \n [3]  Obtener Banners \n [4]  DNS Lookup \n [5]  Subnet Calculator \n [6]  NMAP Port Scan \n [7]  Subdomain Scanner \n [8]  Reverse IP Lookup & CMS Detection \n [9]  SQLi Scanner$white (Finds Links With Parameter And Scans For Error Based SQLi)$blue \n [10] Bloggers View$white (Information That Bloggers Might Be Interested In)$blue \n [11] WordPress Scan$white (Only If The Target Site Runs On WP)$blue \n [12] Crawler \n [13] MX Lookup \n$blue [A]  Scan For Everything - (The Old Lame Scanner) \n$blue [F]  Fix (Checks For Required Modules and Installs Missing Ones) \n$fgreen [U]  Check For Updates \n$white [B]  Scan Another Website (Back To Site Selection) \n$yellow [√] Create BY DeadLikado  Hack\n$green [D] INFO DEL SCRIPT \n$red [Q]  Salir! \n\n" . $cln;
askscan:
    userinput("Escoja Una Opción De La Lista!! >");
	...
	'D',
    ...
    else
      {
        ...
        elseif ($scan == 'q' | $scan == 'Q')
          {
            echo "\n\n\t S C A N  H A C K Es una herramienta para localizar IP ,Informacion de Paginas Entre otras cosas ...... E X I T  Good Bye - Buen Dia:)\n\n";
	    die();
          }
        ...
        elseif ($scan == "0")
          {
            ...
            echo "\n$cln" . $lblue . $bold . "[+] Iniciando Scaneo ... \n";
            echo $blue . $bold . "[i] Scaneando Sitio:\e[92m $ipsl" . "$ip \n";
            echo $bold . $yellow . "[S] Scan Type : SCANEO BASICO" . $cln;
            echo "\n\n";
            echo $bold . $lblue . "[iNFO] Nombre del Sitio: " . $green;
            echo getTitle($reallink);
            echo $cln;
            $wip = gethostbyname($ip);
            echo $lblue . $bold . "\n[iNFO] Direccion IP: " . $green . $wip . "\n" . $cln;
            ...
            echo "\n\n";
            echo $bold . $green . "[*] Scaneo completo presiona Enter o Continue con CTRL + C para parar\n\n";
        ...
        elseif ($scan == "5")
          {
            ...
            $lwwww    = str_replace("www.", "", $ip);
            echo "\n$cln" . $lblue . $bold . "[+] Scanning Begins ... \n";
            echo $blue . $bold . "[i] Scaneando:\e[92m $ipsl" . "$ip \n";
        ...
        elseif ($scan == "7")
          {
            ...
            echo $bold . $yellow . "[*] Scaneo Completo Enter Para Continuar o CTRL + C Para Stop\n\n";
            trim(fgets(STDIN, 1024));
            goto scanlist;
          }
        elseif ($scan == "6")
          {
            $reallink = $ipsl . $ip;
            $lwwww    = str_replace("www.", "", $ip);
            echo "\n$cln" . $lblue . $bold . "[+] Iniciando Escaneo ... \n";
            echo $blue . $bold . "[i] Scaneando sitio:\e[92m $ipsl" . "$ip \n";
            echo $bold . $yellow . "[S] Scan Tipo: Nmap Port Scan" . $cln;
            echo $bold . $lblue . "\n[~] Port Scan Result: \n\n" . $cln;
            $urlnmap    = "http://api.hackertarget.com/nmap/?q=" . $lwwww;
            $resultnmap = readcontents($urlnmap);
            echo $bold . $fgreen . $resultnmap;
            echo "\n\n";
            echo $bold . $yellow . "[*] Scaner Completado Enter o Continue con CTRL + C To Stop\n\n";
            trim(fgets(STDIN, 1024));
            goto scanlist;
          }
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want HACK SCAN To Detect CMS Of The Sites? [Y/N]");
            ...
            if (strpos($srccd, "wp-content") !== false)
              {
                ...
                if ($wp_version != "")
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] HACK SCAN could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ HACK SCAN Update Corner]-\n\n" . $cln;
            ...
            if ($latestversion > $rhversion)
              {
                echo $bold . $fgreen . "\n\n[U] Update Available, Please Update Your Version Of HACK SCAN \n" . $cln;
                echo $bold . $white . "    Link: https://github.com/Tuhinshubhra/RED_HAWK\n\n" . $cln;
              }

            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of HACK SCAN. \n\n" . $cln;
              }
            else
              {
        ...
        elseif ($scan == "F" || $scan == "f"){
          ...
          echo $bold . $fgreen . "[i] Trabajo Hecho!!! Por favor regresa a S C A N   H A C K \n";
          exit;
        }
        elseif ($scan == "A" || $scan == "a")
          {
            ...
            echo "\n\n$bold" . $lblue . "H T T P  p H E A D E R S\n";
            echo "=======================";
            echo "\n\n$cln";
